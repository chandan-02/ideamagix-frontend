                ...
                <tbody>
                    {
                        tableData?.map((itm, i) => {
                            return (<tr key={itm?._id}>
                                ...
                                <td>{itm?.name}</td>
                                <td>{itm?.username}</td>
                                <td className='capitalize'>{itm?.role}</td>
                                <td className='flex gap-3 items-center'>
                                    <PencilSquareIcon onClick={() => {functions[1](itm);window.user_edit_modal.showModal()}} className='h-5 w-5 text-blue-500 cursor-pointer' />
