                ...
                <tbody>
                    {
                        tableData?.map((itm, i) => {
                            return (<tr key={itm?._id}>
                                <th>{i + 1}</th>
                                <td>{itm?.course?.name}</td>
                                <td>{itm?.instructor?.name}</td>
                                <td>{moment(itm?.date).format('DD MMM YYYY')}</td>
                                <td className='flex gap-3 items-center'>
                                    <PencilSquareIcon onClick={() => { functions[1](itm); window.lecture_edit_modal.showModal() }} className='h-5 w-5 text-blue-500 cursor-pointer' />
